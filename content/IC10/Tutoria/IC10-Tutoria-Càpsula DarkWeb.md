---
{"publish":true,"title":"Càpsula Darkweb","tags":["apunts","ic10/tutoria"],"cssclasses":""}
---

# Llicència
Aquest document es publica sota llicència **Creative Commons 3.0 (BY - NC - SA)**

[Creative Commons 3.0 (BY - NC - SA)](https://creativecommons.org/licenses/by-nc-sa/3.0/es/legalcode.ca)

**2025 Raul Gimenez Herrada**
(raul.gimenez@lacetania.cat)

[Ko-Fi Raul Gimenez Herrada - Convida'm a un cafè!](https://ko-fi.com/raulgimenezherrada)

---
# Càpsula DarkWeb
- Tipus de xarxes.
- Com connectar.
- Continguts que s'hi poden trobar.
- Grau d'anonimat.
---
# Tipus de Xarxes
- SurfaceWeb (4%): Accessible desde navegadors.
- DeepWeb (90%): Accessible amb credencials.
- DarkWeb (6%): Accessible amb programari especial i/o configuracions.

[IMATGE WEB](https://www.computerweekly.com/rms/onlineimages/whatis-dark_web_mobile.png)

---
# DarkWeb
Les més conegudes:
- TOR: Sortida SurfaceWeb + hidden services.
- I2P: Hidden services.
- FreeNet: Hidden services.
---
## Tor: Com connectar
- Tor Bundle
- Tor + FoxyProxy (Localhost socks 9050)
---
### Tor: Verificació
- Comprovar IP de sortida abans i després proxy Tor ([WhatIsMyIP](whatismyip.com)).

---
### Tor: Onion Services
- Web ([Hidden Wiki](http://6nhmgdpnyoljh5uzr5kwlatx2u3diou4ldeommfxjz3wkhalzgjqxzqd.onion/))
- Email
- IRC (weechat)
- etc..
---
### Tor: Vulnerable
- Tor no és tant anònim com sembla.
- DNS
[Esquema de funcionament](https://upload.wikimedia.org/wikipedia/commons/1/14/Wat_is_Tor_%28The_onion_routing%29%3F.png)
[Enpaquetat de la info](https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQpXbt_SICFBYcb2aFg-SuaBA099EpZAI7S7Q&s)

---
## I2P: Com connectar
- Instal·lar I2P.
- FoxyProxy
---
### I2P: Hidden Services
- Web (Address book) 
- IRC (127.0.0.1/6668 sense TLS)
- Email
- Web
- Etc...
---