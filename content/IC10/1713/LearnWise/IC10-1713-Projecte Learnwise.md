...
- Posar en pràctica **estratègies d'organització, coordinació i productivitat** (_reunions stand-up, sprints, treball per blocs i pomodoro_)
...
Una _startup_ és una organització temporal creada per buscar un model de negoci repetible i escalable en condicions d’incertesa.
...
L'objectiu principal del mètode _Lean Startup_ és accelerar aquest cicle per generar _aprenentatge validat_ ràpidament i per tant adaptar el producte a la realitat del mercat.
...
Aquí os on desareu tots els documents i coses que genereu durant el projecte.

# TASCA (RA2) - Les hipòtesis: punt de partida del projecte

Abans de començar a crear res, **hem de tenir clar què pensem que passarà**. Aquestes suposicions s’anomenen **hipòtesis** i són la base del mètode _Lean Startup_.

## Què cal tenir en compte per formular una hipòtesi?

### Simplicitat i claredat

Una hipòtesi ha de ser **curta i fàcil d’entendre**.
Exemple: _“Els alumnes voldran utilitzar una eina per recordar exàmens i tasques.”_

### Ser comprovable

Ha de poder-se verificar amb dades reals.
Exemple: _Mètrica clara: “Com a mínim 10 alumnes utilitzaran l’eina durant la primera setmana.”_

### Relacionada amb l’usuari

Les hipòtesis han de parlar del que **faran o voldran les persones que utilitzaran el producte**.
Exemple: _“Els professors trobaran útil tenir informes automàtics de seguiment.”_

### Quantificable

Afegir un número ajuda a **saber si la hipòtesi és certa o falsa**.
Exemple: _“El 50% dels alumnes que provin l’eina tornaran a utilitzar-la la setmana següent.”_

## Quantes hipòtesis cal fer?

Amb 2 o 3 hipòtesis clares n’hi ha prou per començar.  I una d'elles ja la teniu:

**ELS ACTOR EDUCATIUS (ESTUDIANTS, MESTRES I EQUIP DIRECTIU) VOLEN APRENDRE I APLICAR EINES D'IA.**

Les altres dues hipòtesis poden enfocar-se a altres característiques del producte, com per exemple el format, canal de distribució, el procés que es vol millorar aplicant IA, etc.

Aquestes hipòtesis serviran per dissenyar el Producte Mínim Viable (PMV), que ens permetrà comprovar si realment són correctes.

Recorda: una hipòtesi no és una certesa. És un supòsit que cal posar a prova. Encara que resulti falsa, ens ajuda a aprendre i a millorar la idea.

## Entrega

Per dur a terme la tasca cal que com a grup consensueu **dues hipòtesis més**, addicional-ment a la donada, i que **en conjunt siguin diferents de les fetes per altres grups**.   Per tant cal que també us coordineu amb la resta d'equips.

# TASCA (RA2) - Establir les mètriques

Tenint en compte les hipòtesis que heu formulat, el vostre equip ha de definir les mètriques que utilitzarà per corroborar-les.

Per cada hipòtesi caldrà entregar:

- La mètrica principal (North-Star) que demostri el valor esperat.
- 1–2 mètriques secundàries per afegir context (qualitat, satisfacció, operatives).
- El topall d’èxit (x ≥ ?) i el període de mesura (que serà fix a 15 dies).
- La definició exacta de cada mètrica (fórmula, unitats i què compta/no compta).
- El mètode de recollida de dades (quins instruments).
- La regla de decisió (què fem si s’aconsegueix o no el topall).
- Guió d'entrevista que utilitzareu per entrevistar posteriorment mínim 2 usuaris.

## Quines característiques han de tenir les bones mètriques?

Una bona mètrica ha de ser:

- Mesurable: es pot calcular amb dades reals i replicables.
- Accionable: portar a una decisió — no només mostrar un número.
- Rellevant: està directament lligada a la hipòtesi i al valor que volem provar.
- Clara i no ambigua: tothom entén què compta i què no.
- Tenir un topall realista: ser assolible en el context del PMV per tal de prendre decisions.
- Resistent a manipulacions: difícilment “inflable” amb accions artificials.
- Complementada amb qualitat: combinar dades quantitatives amb 2–3 entrevista qualitatives.
...
Remarcar també que el PMV no és "un únic producte o objecte" sinó que és un conjunt, la "solució" global que s'ofereix al client.  Per exemple, si fem un recurs que sigui un curs amb video-tutorials, el PMV seràn els video-tutorials + el email d'invitació + Moodle + SrvWeb + enquestes + entrevistes... **El PMV és tot el que ens calgui per oferir el servei i obtenir aprenentatge validat**.
...
Quan analitzem les mètriques i fem les entrevistes de qualitat és possible que detectem errors en aquest flux i que haguem de tenir present per retorcar-ho i millorar l'experiència dels nostres clients.

### Exemple de flux

_Enviem email d'invitació a un curs al correu corporatiu del client -> Client clica al enllaç i accedeix al Moodle mitjançant correu corporatiu -> Accés al recurs d'aprenentatge -> accedeix a altres recursos d'aprenentatge d'altres companys continguts també al mateix Moodle -> realitza l'enquesta de satisfacció -> Entrevistes qualitatives a dos clients després del periode de proves_

## Implementació

Finalment cal definir com cristal·litzen aquestes característiques en un producte o servei concret, i també que ens caldrà per fer-ho.  **Aquí ja cal concretar què cal fer a nivell tècnic, quins recursos necessitem i qui ho farà**.  Pot ser útil utilitzar el _flux de l'experiència_ com a guia.
...
Aquesta part de coordinació la durà a terme principalment el _Product Owner_ com a responsable del projecte. De cada reunió caldrà aixecar acte (escriure-la) al _Drive_ del grup en un document anomenat _Diari de desenvolupament_, que s'utilitzarà per valorar aquesta tasca.
...
- Detectar si han aparegut _blockers_.
...
Si una tasca ocupa més que el temps que teniu avuí, per exemple _Muntar un servidor de n8n_, l'heu de partir en subtasques que si puguin encabir en el temps disponible. Així doncs _Muntar un servidor n8n_ es podria desglossar en _Muntar la VM i instal·lar el S.O._ + _Instal·lar i configurar n8n_.

D'aquesta forma queda clar que s'ha de fer en aquesta sessió i si no s'acaba la tasca serà una _desviació_ respecte al que heu planificat i caldrà veure entre tots el motiu: _¿Heu planificat malament? ¿Han sorgit imprevistos, quins? ¿No s'ha treballat prou avuí?_
...
En les sessions de 4 hores, al mitja sessió (2 hores) fer una segona reunió _stand-up_ per simplement verificar el progrés de la feina.

Cal que no us acomodeu i procureu que la feina surti, ja que us hi va la nota!

D'aquesta reunió no és imprescindible fer acta, ja que segurament quedarà reflectida en la reunió _stand-up_ de la propera sessió.
...
Arribats aquí acabem un cicle del _mètode lean startup_ amb la decisió de si pivotem o perseverem.  Hem d'utilitzar les mètriques i mecanismes de decisió que havíem establert al principi per prendre la decisió però en tot cas aquí ens caldria resumir:
